...
export const ScannerModal: React.FC<ScannerModalProps> = ({ apiKey, onClose, onScanSuccess }) => {
  ...
  const startCamera = useCallback(async () => {
    // Don't start camera if there's already an image from upload
    if (imageSrc) return;
    try {
                ...
                {imageSrc ? (
                  <>
                   ...
                   <div className="flex items-center justify-center gap-6">
                      <button onClick={handleCapture} title="使用相機拍攝" className="w-20 h-20 rounded-full font-bold text-white bg-light-primary dark:bg-dark-primary hover:opacity-90 transform hover:scale-105 transition-all flex items-center justify-center ring-4 ring-white/20">
                        <Icon icon="camera" className="w-8 h-8"/>
                      </button>
                      <button onClick={handleUploadClick} title="從檔案上傳" className="w-20 h-20 rounded-full font-bold text-light-text-secondary dark:text-dark-text-secondary bg-light-border/50 dark:bg-dark-border/50 hover:bg-light-border dark:hover:bg-dark-border transform hover:scale-105 transition-all flex items-center justify-center ring-4 ring-white/10 dark:ring-black/10">
                        <Icon icon="upload" className="w-8 h-8"/>
                      </button>
                    </div>
                )}
              </div>
